...
def load_config(path: Path) -> dict[str, Any]:
    ...
    required = {"experiment", "paths", "data", "splitting", "training", "evaluation"}
    missing = required - set(cfg)
    if missing:
        raise ConfigurationError(f"Missing top-level YAML sections: {sorted(missing)}")

    for key in ("htc_root", "data_root", "runs_root"):
    ...
    if not isinstance(sources, dict) or len(sources) < 2:
        raise ConfigurationError("data.sources must define at least two label-to-folder mappings")
