...
The two supported preprocessing configurations are deliberately paired:

- `use_L1pixel_normalized_values: false` requires `training.standardize: true`;
- `use_L1pixel_normalized_values: true` requires `training.standardize: false`.

The pipeline rejects either contradictory pairing before preparing or training a run. Because both variants load precomputed CSV spectra, HTC does not normalize the model input or perform its raw-pixel L1-sum check. The L1 setting describes the upstream pixel processing used to produce the selected median-spectrum CSV.
...
Completed combination folders use the testing accuracy followed by the combination run number, for example `097_12345678_000000000000000000000000000012` for run 12 with 97.12345678% testing accuracy. The aggregate `combined_results.csv` and `combined_results.xlsx` files place the ordinary numeric run number in the first column and sort completed runs from lowest to highest testing accuracy. They do not contain a separate rank column.
...
The source path must contain a supported subject token such as `P<number>`, `M<number>`, or `Cat_<number>_...`. The folder immediately below a directory named `data` may also use `YYYY_MM_DD_<subject>`, for example `data/2021_01_10_SPACE_000107`, which resolves to subject `SPACE_000107`. Correct unsupported source-folder conventions or extend `subject_from_path()` in `median_pipeline/preparation.py` deliberately and test the change.
