...
Choose the spectrum CSV source with one YAML value:

```yaml
data:
  use_L1pixel_normalized_values: false
```

`false` loads the existing `spectrum_fromCSV1_(500.0-995.0)_masked_data_0_derivative.csv` files. To use the L1-pixel spectra instead, turn the switch on:
...
When `true`, the pipeline only loads `_L1pixel/spectrum_fromCSV1_masked_data_L1pixel_0_derivative.csv`; an identically named CSV outside an `_L1pixel` folder is ignored. The switch applies to both the primary and external-testing datasets. Existing configurations without `use_l1pixel` remain compatible: explicit `data.patterns` continue to work as custom patterns, while configurations without patterns default to the normal median CSV.
...
external_testing:
  ...
  labelling_file:
    rat_stomach: "_labelling_001.txt"
    rat_liver: "_labelling_001.txt"

  hyperguis:
    rat_stomach: "_hypergui_1"
    rat_liver: "_hypergui_1"
```

The external classes are independent from the primary training classes. The keys only need to match across the three external mappings (`experiment_folders`, `labelling_file`, and `hyperguis`). Every external experiment folder must be an absolute path. A folder can be a single path for the current platform or contain separate `windows` and `linux` paths as shown above.

With external testing enabled, the configured primary `train_ratio` and `val_ratio` are normalized to use all primary subjects. For example, `0.70/0.15/0.15` becomes approximately `82.35%` training and `17.65%` validation. The primary dataset contributes no test rows; 100% of the external dataset becomes the test set. Feature standardization is still calculated only from primary training rows. The pipeline rejects a different external wavelength grid or overlapping spectrum files. Set `enabled: false` to restore the original single-dataset train/validation/test split.

External evaluation uses independent axes: matrix rows are the external true classes and matrix columns are the model's training classes. Therefore three external classes tested with a nine-class model produce a `3 × 9` matrix. Accuracy and per-class recall count a prediction as correct only when the external class name exactly matches a trained class name; external-only class names correctly receive zero recall. The external labels are used only for evaluation and are never passed into the trained model's loss function.

### Multiple selective test cases

To run several independent selections with one command, make `selective` a mapping. Each mapping key becomes its child-folder name, and commas or semicolons may separate exact wavelengths:
...
The data are discovered once and all cases reuse the same subject split. Runs execute sequentially in folders named with the test-case key directly below the master scenario. For example, `selective: {1: "450; 510"}` writes to `<master scenario>/1/`. This keeps paths portable even when a case contains dozens of wavelengths. The full selection remains recorded in the child configuration, status file, and aggregate results. Each child contains its own `output/` and `run_config/`; the master `splits/` folder is shared. Progress is stored in `selective results/status.json`, and completed training runs are summarized in `combined_results.csv` and `combined_results.xlsx`. Rerunning the master skips children that already contain `run_config/run_parameters.json`.
...
Combination analysis is an optional batch mode which trains every mathematical combination of a requested size within an inclusive wavelength range. Enable it explicitly and leave the other wavelength batch modes disabled:
...
The candidates are the wavelengths actually available on the discovered CSV grid between `combination_analysis.min` and `combination_analysis.max`, inclusive. The pipeline generates every unordered combination without repetition. For example, 500–700 nm on a 5 nm grid contains 41 candidates, so selecting three produces `41 choose 3 = 10,660` complete training runs. Data discovery and the subject split are performed once and reused by every child.

Children are stored under `combination runs/`. A child uses a temporary name while it is running and is renamed after evaluation using only its zero-padded test accuracy with eight decimal places. For example, a final test accuracy of `93.12345678%` produces the folder `093_12345678`. Windows Explorer can show the highest final performance first by sorting names in descending order. The wavelength selection is stored in `run_config/combination_run.json`, allowing completed children to be detected and skipped when the master is resumed. If two runs have the same test accuracy at eight decimal places, the pipeline reports a collision instead of overwriting an existing result.

`combination results/combined_results.csv` and `combination results/combined_results.xlsx` contain all completed runs ranked permanently from highest to lowest validation accuracy. Full-precision validation and testing metrics, selected wavelengths, status, and final run paths are included. Set `combination_analysis.enabled: false` to use ordinary single, selective, or stepwise scenarios unchanged.

To require one or more wavelengths in every combination, list them under `fixed_wavelengths`. `n_wavelengths` remains the total number of channels per run. For example, this configuration fixes 700 nm and tries every possible pair from 500–695 nm:
...
The resulting selections are named `700+500+505`, `700+500+510`, and so on. Fixed wavelengths must exist on the available CSV grid and may lie outside the variable `min`/`max` search range, but must remain inside the overall `wavelength.min`/`wavelength.max` range. Multiple fixed wavelengths are supported, such as `fixed_wavelengths: [650, 700]` with `n_wavelengths: 3` to vary only the third channel.
...
Every organ workbook contains `Train Details`, `Validation Details`, and `Test Details`. Each matrix workbook contains `Matrix Normalized` and `Matrix Counts`. The `training_matrices/` folder contains matrix CSV files, raw and normalized light-theme PNGs, and `confusion_matrix.xlsx`; it does not contain dark-theme plots or PDFs. Validation and testing matrices retain their configured PNG/PDF light/dark outputs. Normalized values are fractions from 0 to 1 displayed with three decimal places, and every plot title includes that split's accuracy. Each run's `loss_curves/` folder contains separate training and validation cross-entropy curves plus the per-epoch values in `loss_history.csv`. The run root contains one-row `validation_metrics.csv` and `testing_metrics.csv` files with the summary metrics for each split. Only the testing normalized light/dark PDFs are copied from `testing_matrices/pdf/` into the run root. The testing `summary.json` is also written in the run root.
