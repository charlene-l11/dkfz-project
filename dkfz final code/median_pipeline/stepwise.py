...
from median_pipeline.wavelengths import parse_selective, resolve_wavelength_selection
...
def _selection_name(available: np.ndarray, indices: tuple[int, ...]) -> str:
    """Compact selected channels into names such as ``500-515+580+600-610``."""
    ...
    for start, end in groups:
        lower = _format_wavelength(float(available[start]))
        upper = _format_wavelength(float(available[end]))
        parts.append(lower if start == end else f"{lower}-{upper}")
    return "+".join(parts)
...
def build_stepwise_runs(cfg: dict[str, Any], wavelength_info: dict[str, Any]) -> list[StepwiseRun]:
    ...
    intervals = parse_selective(cfg["wavelength"].get("selective"))
    spacing = float(np.median(np.diff(available)))
    tolerance = max(1e-6, abs(spacing) * 0.01)

    configured_indices = wavelength_info.get("selected_indices")
    if configured_indices is not None:
        selected_indices = tuple(int(index) for index in configured_indices)
    elif intervals:
        selected_indices = tuple(
            int(index)
            for index, value in enumerate(available)
            if any(lower - tolerance <= value <= upper + tolerance for lower, upper in intervals)
        )
    else:
        selected_indices = tuple(range(len(available)))

    if len(selected_indices) < 2:
        raise ConfigurationError("Stepwise analysis requires at least two selected wavelength channels")

    runs: list[StepwiseRun] = []
    if cfg["stepwise_analysis"]:
        forward_stop = float(cfg["forward_stop_wavelength"])
        forward_indices = tuple(
            index for index in selected_indices if available[index] <= forward_stop + tolerance
        )
        if len(forward_indices) < 2:
            raise ConfigurationError(
                "forward_stop_wavelength must include at least two selected wavelength channels"
            )
        for count in range(2, len(forward_indices) + 1):
            indices = forward_indices[:count]
            lower, upper = float(available[indices[0]]), float(available[indices[-1]])
            runs.append(
                StepwiseRun(
                    direction="forward",
                    range_name=_selection_name(available, indices),
    ...
    if cfg["reverse_stepwise"]:
        ...
        reverse_indices = tuple(
            index for index in selected_indices if available[index] >= reverse_stop - tolerance
        )
        if len(reverse_indices) < 2:
            raise ConfigurationError(
                "reverse_stop_wavelength must include at least two selected wavelength channels"
            )
        for count in range(2, len(reverse_indices) + 1):
            indices = reverse_indices[-count:]
            lower, upper = float(available[indices[0]]), float(available[indices[-1]])
            runs.append(
                StepwiseRun(
                    direction="reverse",
...
def _child_config(master_cfg: dict[str, Any], master_config: Path, run: StepwiseRun) -> dict[str, Any]:
    child = json.loads(json.dumps(master_cfg))
    child["stepwise_analysis"] = False
    child["reverse_stepwise"] = False
    if "forward_stepwise_analysis" in child:
        child["forward_stepwise_analysis"] = False
    if "reverse_stepwise_analysis" in child:
        child["reverse_stepwise_analysis"] = False
...
def run_stepwise(
    ...
    manifest, mapping, split_summary, base_wavelength_info = prepare_data(cfg)
