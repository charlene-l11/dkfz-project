...
def _completed_folder_name(
    run: CombinationRun,
    test_accuracy: float,
    run_number: int,
) -> str:
    if not np.isfinite(test_accuracy) or not 0 <= test_accuracy <= 1:
        raise ConfigurationError(
            f"Test accuracy must lie between 0 and 1, got {test_accuracy!r}"
        )
    if (
        isinstance(run_number, bool)
        or not isinstance(run_number, int)
        or not 1 <= run_number < 10**30
    ):
        raise ConfigurationError(
            f"Combination run number must be an integer from 1 to {10**30 - 1}, got {run_number!r}"
        )
    # Keep completed folders sortable while avoiding punctuation that is awkward
    # in paths. For example, run 12 with 97.12345678% becomes
    # ``097_12345678_000000000000000000000000000012``.
    accuracy = f"{test_accuracy * 100:012.8f}".replace(".", "_")
    return f"{accuracy}_{run_number:030d}"
...
def _combination_identity_path(child_root: Path) -> Path:
    return child_root / "run_config" / "combination_run.json"


def _write_combination_identity(child_root: Path, run: CombinationRun) -> None:
    path = _combination_identity_path(child_root)
...
def _selection_from_identity(child_root: Path) -> str | None:
    path = _combination_identity_path(child_root)
    if not path.is_file():
        return None
    try:
        selection = json.loads(path.read_text(encoding="utf-8")).get("selection")
...
def _discover_roots(runs_root: Path) -> dict[str, Path]:
    ...
    for path in runs_root.iterdir():
        if not path.is_dir():
            continue
        # Pending and legacy folders encode the selection in their name.  New
        # completed folders intentionally contain only the accuracy, so their
        # selection is read from the run metadata instead.
        selection = _selection_from_folder_name(path.name) or _selection_from_identity(path)
...
def _finalize_folder(
    ...
    test_accuracy = float(metrics["testing_accuracy"])
    final_root = runs_root / _completed_folder_name(run, test_accuracy, run_number)
    _write_combination_identity(child_root, run)
    if child_root != final_root:
        if final_root.exists():
            existing_selection = _selection_from_identity(final_root)
            detail = (
                f" for wavelength selection {existing_selection!r}"
                if existing_selection is not None
                else ""
            )
            raise FileExistsError(
                f"Combination result folder already exists{detail}: {final_root}"
            )
...
def _status_record(run: CombinationRun, run_number: int, child_root: Path) -> dict[str, Any]:
...
def write_aggregate_results(results_root: Path, records: list[dict[str, Any]]) -> None:
    rows = [record.copy() for record in records if record.get("testing_accuracy") is not None]
    if not rows:
        return
    frame = pd.DataFrame(rows)
    frame = frame.sort_values(
        ["testing_accuracy", "run_number"], ascending=[True, True], kind="stable"
    )
    run_numbers = frame.pop("run_number")
    frame.insert(0, "run_number", run_numbers)
...
def run_combinations(
    ...
    for run_number, run in enumerate(runs, start=1):
        ...
        if existing is None:
            continue
        if (existing / "run_config" / "run_parameters.json").is_file():
            finalized, _ = _finalize_folder(runs_root, run, run_number, existing)
    ...
    for number, (run, record) in enumerate(zip(runs, records, strict=True), start=1):
        ...
        try:
            ...
            if prepare_only:
                ...
                child_root, metrics = _finalize_folder(runs_root, run, number, child_root)
