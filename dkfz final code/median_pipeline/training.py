...
def _precomputed_input_settings(use_l1pixel: bool) -> dict[str, Any]:
    """Describe CSV inputs without asking HTC to normalize or validate them again."""
    return {
        "normalization": None,
        "preprocessing": None,
        "precomputed_pixel_normalization": "L1" if use_l1pixel else "none",
    }
...
def run_training(cfg: dict[str, Any], run_dir: Path, run_config_dir: Path) -> dict[str, Any]:
    ...
    # Both input variants are precomputed CSV spectra. In the L1 case, the
    # normalization happened per pixel upstream, before the median was taken.
    # Disabling the runtime L1 expectation prevents a second/invalid check on
    # the resulting median spectrum while preserving the source in run metadata.
    for key, value in _precomputed_input_settings(
        bool(cfg["data"]["use_L1pixel_normalized_values"])
    ).items():
        config[f"input/{key}"] = value
