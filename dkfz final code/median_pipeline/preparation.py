...
def prepare_data(
    ...
    testing, testing_mapping, testing_wavelength_info = discover(testing_cfg)

    primary_labels = set(mapping)
    testing_labels = set(testing_mapping)
    if testing_labels != primary_labels:
        raise ValueError(
            "External testing labels must exactly match the discovered training labels; "
            f"missing={sorted(primary_labels - testing_labels)}, "
            f"extra={sorted(testing_labels - primary_labels)}"
        )
