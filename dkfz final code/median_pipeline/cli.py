...
def create_run_dir(cfg: dict) -> Path:
    ...
    for folder in ("config", "data", "checkpoints", "logs", "predictions", "matrices/csv", "matrices/png", "matrices/pdf", "metrics"):
...
def run(config_path: Path, prepare_only: bool = False) -> Path:
    ...
    print(f"[1/3] Preparing median-spectrum CSV data in {run_dir}")
    ...
    print("[2/3] Training the HTC median-pixel model")
    from median_pipeline.training import run_training
    run_training(cfg, run_dir)
    print("[3/3] Evaluation and confusion matrices complete")
