...
from median_pipeline.wavelengths import scatter_selected_features
...
class DatasetCatPigMedianPixel(HTCDataset):
    def __init__(
        ...
        original_channel_count = int(features.shape[1])
        wavelength_indices = config.get("input/wavelength_indices")
        indices: np.ndarray | None = None
        if wavelength_indices is not None:
            indices = np.asarray(wavelength_indices, dtype=np.int64)
            if indices.ndim != 1 or len(indices) == 0:
                raise ValueError("input/wavelength_indices must contain at least one channel index")
            if indices.min() < 0 or indices.max() >= features.shape[1]:
                raise ValueError("A selected wavelength index lies outside the loaded spectrum")
            features = features[:, indices]

        feature_mean = config.get("input/feature_mean")
        feature_std = config.get("input/feature_std")
        if feature_mean is not None and feature_std is not None:
            mean = np.asarray(feature_mean, dtype=features.dtype)
            std = np.asarray(feature_std, dtype=features.dtype)
            if mean.shape != (features.shape[1],) or std.shape != (features.shape[1],):
                raise ValueError("Feature mean and standard deviation must match the selected channel count")
            std = std.copy()
            std[std == 0] = 1
            features = (features - mean) / std

        mask_unselected = bool(config.get("input/mask_unselected_wavelengths", False))
        if mask_unselected:
            if indices is None:
                raise ValueError("Fixed-width wavelength masking requires input/wavelength_indices")
            model_channel_count = int(config["input/n_channels"])
            if model_channel_count != original_channel_count:
                raise ValueError(
                    "Fixed-width model input must match the number of channels in the loaded spectrum "
                    f"({model_channel_count} configured, {original_channel_count} loaded)"
                )
            features = scatter_selected_features(features, indices, model_channel_count)

        self.features = torch.from_numpy(features)

        self.features = cast(torch.Tensor, self.apply_transforms(self.features))
        if mask_unselected:
            # Keep masked wavelengths neutral even if a configured transform modifies zero values.
            unselected = torch.ones(self.features.shape[1], dtype=torch.bool)
            unselected[torch.as_tensor(indices, dtype=torch.long)] = False
            self.features[:, unselected] = 0
