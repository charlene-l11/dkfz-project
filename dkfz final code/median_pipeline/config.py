...
def load_config(path: Path) -> dict[str, Any]:
    ...
    for key in ("stepwise_analysis", "reverse_stepwise"):
    ...
    if cfg["stepwise_analysis"] or cfg["reverse_stepwise"]:
        if intervals:
            raise ConfigurationError(
                "wavelength.selective must be null when forward or reverse stepwise analysis is enabled"
            )
