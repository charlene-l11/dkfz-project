...
from median_pipeline.preparation import (
    discover,
    prepare_data,
    split_manifest,
    subject_from_path,
...
class SplittingTests(unittest.TestCase):
    def test_subject_identifiers_from_supported_windows_data_paths(self):
        paths = {
            PureWindowsPath(
                r"Z:\TIVITA_Cat\Cat_Pat\Cat_tx_liver\Cat_tx_liver_1_prior_to_reperf"
                r"\data\2021_01_10_SPACE_000107\2021_01_10_21_46_01\_hypergui_1\spectrum.csv"
            ): "SPACE_000107",
            PureWindowsPath(
                r"Z:\TIVITA_Cat\Cat_Pat_LED_UMM\Cat_atlas_LED\Cat_atlas_bladder_LED"
                r"\data\P0008_2023_10_10_Experiment1\_hypergui_1\spectrum.csv"
            ): "P0008",
            PureWindowsPath(
                r"Z:\TIVITA_Cat\Cat_Pat_Mini_UMM\Cat_atlas_Mini\Cat_atlas_mini_fossa"
                r"\data\M0001_2024_01_31_Experiment1\_hypergui_1\spectrum.csv"
            ): "M0001",
            PureWindowsPath(
                r"Z:\TIVITA_Cat\Cat_Mibi\Cat_material_wb_2wb_check_backgr_2wb_check"
                r"\Cat_medium_4MH\data\P002_OP002_2026_07_20_Experiment1"
                r"\_hypergui_1\spectrum.csv"
            ): "P002",
        }

        for path, expected in paths.items():
            with self.subTest(path=path):
                self.assertEqual(subject_from_path(path), expected)
    ...
    def test_external_labels_are_written_with_an_independent_mapping(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_prepared(
                ...
                summary,
                {
                    "configured_min": 500.0,
                    "configured_max": 505.0,
                    "selective": 500.0,
                    "grid_source": "csv_wavelength_column",
                    "available_wavelengths": [500.0, 505.0],
                    "selected_indices": [0],
                    "selected_wavelengths": [500.0],
                    "n_input_channels": 2,
            ...
            wavelength_json = json.loads(
                (data_dir / "wavelengths.json").read_text(encoding="utf-8")
            )
            wavelength_text = json.loads(
                (output_dir / "wavelengths.txt").read_text(encoding="utf-8")
            )
            wavelength_book = load_workbook(output_dir / "wavelengths.xlsx", data_only=True)
            self.assertEqual(wavelength_text, wavelength_json)
            self.assertEqual(wavelength_book.sheetnames, ["Overview", "Wavelengths"])
            self.assertEqual(wavelength_book["Wavelengths"]["B2"].value, 500.0)
            self.assertEqual(wavelength_book["Wavelengths"]["C2"].value, "Yes")
            self.assertEqual(wavelength_book["Wavelengths"]["C3"].value, "No")
