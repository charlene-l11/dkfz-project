...
from median_pipeline.preparation import discover, prepare_data, split_manifest
...
class SplittingTests(unittest.TestCase):
    ...
    def test_prepare_data_assigns_second_dataset_only_to_test(self, discover_mock):
        ...
        external = pd.DataFrame([
            {"subject_name": "M001", "label": "kidney"},
            {"subject_name": "M001", "label": "liver"},
        ])
        wavelength_info = {
        ...
        discover_mock.side_effect = [
            ...
            (external, {"kidney": 0, "liver": 1}, wavelength_info),
        ]
        cfg = {
            "external_testing": {
                "enabled": True,
                "experiment_folders": {"kidney": "/external-rat/kidney", "liver": "/external-rat/liver"},
                "labelling_file": {"kidney": "labels.txt", "liver": "labels.txt"},
                "hyperguis": {"kidney": "_hypergui_1", "liver": "_hypergui_1"},
            },
            "splitting": {
                "train_ratio": 0.7,
                "val_ratio": 0.15,
