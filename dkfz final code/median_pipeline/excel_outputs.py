...
def _style_detail_sheet(sheet, title: str, subset: pd.DataFrame) -> None:
    ...
    headers = ["individual / recording", "label_name", "image_name", "sample_dir", "hypergui_dir", "spectrum_csv_path"]
    ...
    for individual, recordings in rows.groupby("_individual", sort=True):
        ...
        for _, record in recordings.iterrows():
            values = [
                ...
                _detail_value(record, "sample_dir"), _detail_value(record, "hypergui_dir"), str(record["file_path"]),
            ]
            for column, value in enumerate(values, start=1):
                sheet.cell(row=row_number, column=column, value=value)
            row_number += 1
    sheet.auto_filter.ref = f"A6:F{max(6, row_number - 1)}"
    for column, width in enumerate([34, 20, 55, 70, 70, 90], start=1):
