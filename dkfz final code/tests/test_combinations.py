...
class CombinationTests(unittest.TestCase):
    ...
    def test_completed_folder_uses_zero_padded_test_accuracy_percentage(self):
        ...
        self.assertEqual(
            _completed_folder_name(run, 0.9312345678, 12),
            "093_12345678_000000000000000000000000000012",
        )
        self.assertEqual(
            _completed_folder_name(run, 0.0525, 1),
            "005_25000000_000000000000000000000000000001",
        )
        self.assertEqual(
            _completed_folder_name(run, 1.0, 999),
            "100_00000000_000000000000000000000000000999",
        )
    ...
    def test_runs_rename_by_test_accuracy_and_results_are_sorted(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                ...
                [
                    "062_50000000_000000000000000000000000000001",
                    "080_00000000_000000000000000000000000000003",
                    "087_50000000_000000000000000000000000000002",
                ],
            )
            ...
            self.assertEqual(combined.columns[0], "run_number")
            self.assertEqual(combined["run_number"].tolist(), [1, 3, 2])
            self.assertEqual(combined["testing_accuracy"].tolist(), [0.625, 0.8, 0.875])
            self.assertNotIn("rank", combined.columns)
            ...
            # Re-running an unchanged analysis skips every completed run.
