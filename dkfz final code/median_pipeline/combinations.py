...
def _completed_folder_name(run: CombinationRun, test_accuracy: float) -> str:
    ...
    # in paths.  For example, 93.12345678% becomes ``093_12345678``.
    return f"{test_accuracy * 100:012.8f}".replace(".", "_")
...
def _finalize_folder(runs_root: Path, run: CombinationRun, child_root: Path) -> tuple[Path, dict[str, Any]]:
    ...
    final_root = runs_root / _completed_folder_name(run, test_accuracy)
    ...
    if child_root != final_root:
        if final_root.exists():
            ...
            raise FileExistsError(
                ...
                "Two runs may have the same test accuracy at eight decimal places."
            )
