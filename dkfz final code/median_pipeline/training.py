...
def run_training(cfg: dict[str, Any], run_dir: Path) -> dict[str, Any]:
    ...
    config["input/n_channels"] = len(wavelength_indices)
    ...
    test_manifest = pd.read_csv(test_path)
    if len(test_manifest) != len(logits):
        raise RuntimeError(f"Test manifest has {len(test_manifest)} rows but HTC returned {len(logits)} predictions")
    predictions = test_manifest.copy()
    predictions["prediction_index"] = logits.argmax(axis=1)
    predictions["prediction_label"] = predictions["prediction_index"].map({i: label for i, label in enumerate(labels)})
    ...
    summary = write_evaluation(predictions, labels, run_dir, cfg)
    resolved_dir = run_dir / "config"
    resolved_dir.mkdir(parents=True, exist_ok=True)
    (resolved_dir / "htc_config.json").write_text(json.dumps(config.data, indent=2), encoding="utf-8")
    run_parameters = {
        "pipeline_config": cfg,
    ...
    (run_dir / "run_parameters.json").write_text(json.dumps(run_parameters, indent=2), encoding="utf-8")
