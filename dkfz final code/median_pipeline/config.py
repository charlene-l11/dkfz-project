...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if "spectrum_source" in data:
        raise ConfigurationError(
            "data.spectrum_source was replaced by the true/false "
            "data.use_L1pixel_normalized_values setting"
        )
    if "use_l1pixel" in data:
        raise ConfigurationError(
            "data.use_l1pixel was replaced by the exact, case-sensitive "
            "data.use_L1pixel_normalized_values setting"
        )
    use_l1pixel = data.get("use_L1pixel_normalized_values")
    if not isinstance(use_l1pixel, bool):
        raise ConfigurationError("data.use_L1pixel_normalized_values must be true or false")
    ...
    combination = cfg.setdefault("combination_analysis", {"enabled": False})
    if not isinstance(combination, dict):
        raise ConfigurationError("combination_analysis must be a YAML mapping")
    combination_enabled = combination.get("enabled", False)
    if not isinstance(combination_enabled, bool):
        raise ConfigurationError("combination_analysis.enabled must be true or false")
    combination["enabled"] = combination_enabled
    if combination_enabled:
        required_combination = {"min", "max", "n_wavelengths"}
        missing_combination = required_combination - set(combination)
        if missing_combination:
            raise ConfigurationError(
                f"Missing combination_analysis settings: {sorted(missing_combination)}"
            )
        combination_minimum = float(combination["min"])
        combination_maximum = float(combination["max"])
        if not minimum <= combination_minimum <= combination_maximum <= maximum:
            raise ConfigurationError(
                "combination_analysis min/max must define an inclusive range inside "
                f"wavelength.min={minimum:g} and wavelength.max={maximum:g}"
            )
        count = combination["n_wavelengths"]
        if isinstance(count, bool) or not isinstance(count, int) or count < 1:
            raise ConfigurationError("combination_analysis.n_wavelengths must be a positive integer")
        fixed = combination.get("fixed_wavelengths", [])
        if isinstance(fixed, (int, float)) and not isinstance(fixed, bool):
            fixed = [fixed]
        if (
            not isinstance(fixed, list)
            or any(isinstance(value, bool) or not isinstance(value, (int, float)) for value in fixed)
        ):
            raise ConfigurationError(
                "combination_analysis.fixed_wavelengths must be a number or list of numbers"
            )
        normalized_fixed = [float(value) for value in fixed]
        if len(set(normalized_fixed)) != len(normalized_fixed):
            raise ConfigurationError("combination_analysis.fixed_wavelengths must not contain duplicates")
        if len(normalized_fixed) > count:
            raise ConfigurationError(
                "combination_analysis cannot define more fixed_wavelengths than n_wavelengths"
            )
        outside = [value for value in normalized_fixed if not minimum <= value <= maximum]
        if outside:
            raise ConfigurationError(
                "combination_analysis.fixed_wavelengths must lie inside the configured wavelength range; "
                f"outside values: {outside}"
            )
        combination["min"] = combination_minimum
        combination["max"] = combination_maximum
        combination["n_wavelengths"] = count
        combination["fixed_wavelengths"] = normalized_fixed

        conflicting_modes = []
        if wavelength["selective"] is not None:
            conflicting_modes.append("wavelength.selective")
        if cfg["stepwise_analysis"]:
            conflicting_modes.append("forward_stepwise_analysis")
        if cfg["reverse_stepwise"]:
            conflicting_modes.append("reverse_stepwise_analysis")
        if conflicting_modes:
            raise ConfigurationError(
                "combination_analysis cannot be combined with " + ", ".join(conflicting_modes)
            )
    ...
    if not isinstance(training["standardize"], bool):
        raise ConfigurationError("training.standardize must be true or false")
    if use_l1pixel and training["standardize"]:
        raise ConfigurationError(
            "L1-pixel spectra must use training.standardize: false when "
            "data.use_L1pixel_normalized_values is true"
        )
    if not use_l1pixel and not training["standardize"]:
        raise ConfigurationError(
            "Original-reflectance spectra must use training.standardize: true when "
            "data.use_L1pixel_normalized_values is false"
        )
