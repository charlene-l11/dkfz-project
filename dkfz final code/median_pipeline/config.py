...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if enabled:
        ...
        for section in required_external:
            if not isinstance(external[section], dict):
                raise ConfigurationError(f"external_testing.{section} must be a mapping")
            section_keys = set(external[section])
            if section_keys != expected_keys:
                raise ConfigurationError(
                    f"external_testing.{section} keys must exactly match the training organ keys; "
                    f"missing={sorted(expected_keys - section_keys)}, "
                    f"extra={sorted(section_keys - expected_keys)}"
                )
