...
Children are stored under `combination runs/`. A child uses a temporary name while it is running and is renamed after evaluation using its validation accuracy, for example `92.34%; 500+505+510`. Windows Explorer can show the highest accuracy first by sorting names in descending order. Completed children are detected and skipped when the master is resumed.

`combination results/combined_results.csv` and `combination results/combined_results.xlsx` contain all completed runs ranked permanently from highest to lowest validation accuracy. Full-precision validation and testing metrics, selected wavelengths, status, and final run paths are included; folder percentages are rounded to two decimal places only for display. Set `combination_analysis.enabled: false` to use ordinary single, selective, or stepwise scenarios unchanged.
