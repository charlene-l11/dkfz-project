...
def create_run_dir(cfg: dict) -> Path:
    path = Path(cfg["paths"]["runs_root"]) / safe_name(str(cfg["experiment"]["name"]))
    if path.exists():
        raise FileExistsError(
            f"Run folder already exists: {path}. Change experiment.name in the YAML or move the existing run."
        )
    path.mkdir(parents=True, exist_ok=False)
    for folder in ("config", "data", "splits", "checkpoints", "logs", "predictions"):
