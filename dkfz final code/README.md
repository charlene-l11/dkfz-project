...
Children are stored under `combination runs/`. A child uses a temporary name while it is running and is renamed after evaluation using only its zero-padded validation accuracy with eight decimal places. For example, an accuracy of `93.12345678%` produces the folder `093_12345678`. Windows Explorer can show the highest accuracy first by sorting names in descending order. The wavelength selection is stored in `run_config/combination_run.json`, allowing completed children to be detected and skipped when the master is resumed. If two runs have the same validation accuracy at eight decimal places, the pipeline reports a collision instead of overwriting an existing result.
