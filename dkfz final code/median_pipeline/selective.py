...
class SelectiveRun:
    ...
    def folder_name(self) -> str:
        return _selection_folder_name(self.selection)


def _selection_folder_name(selection: Any) -> str:
    """Preserve configured wavelength text as a portable child-folder name."""
    if selection is None:
        name = "all"
    elif isinstance(selection, (int, float)):
        number = float(selection)
        name = str(int(number)) if number.is_integer() else f"{number:g}"
    elif isinstance(selection, list):
        name = "; ".join(str(value).strip() for value in selection)
    else:
    ...
    if not name or name in {".", ".."} or windows_unsafe or name.endswith((" ", ".")):
        raise ConfigurationError(
            f"Wavelength selection {selection!r} cannot be used as a Windows-compatible folder name"
        )
    return name
