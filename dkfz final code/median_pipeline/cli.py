...
def run_single(
    ...
    if prepared is None:
        ...
        print("  Reusing the shared master subject split", flush=True)
    print("  Split assignment finished; writing manifests and workbooks...", flush=True)
    write_prepared(manifest, mapping, split_summary, wavelength_info, run_dir)
...
def run(config_path: Path, prepare_only: bool = False) -> Path:
    ...
    if cfg["combination_analysis"]["enabled"]:
        from median_pipeline.combinations import run_combinations

        return run_combinations(config_path, cfg, prepare_only, run_single)
