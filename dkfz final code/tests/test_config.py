...
def base_config():
    return {
        ...
        "experiment_folders": {"a": "/data-root/folder-a", "b": "/data-root/folder-b"},
        "labelling_file": {"a": "_labelling_001.txt", "b": "_labelling_001.txt"},
        "hyperguis": {"a": "_hypergui_1", "b": "_hypergui_1"},
        "data": {"use_L1pixel_normalized_values": False},
...
class ConfigTests(unittest.TestCase):
    ...
    def test_l1pixel_source_resolves_to_nested_l1pixel_csv(self):
        ...
        value["training"]["standardize"] = False
    ...
    def test_l1pixel_switch_must_be_boolean(self):
        value = base_config()
        value["experiment_folders"] = {"a": "/training/a", "b": "/training/b"}
        value["data"]["use_L1pixel_normalized_values"] = "yes"

        with self.assertRaisesRegex(ConfigurationError, "true or false"):
            self.load(value)

    def test_l1pixel_source_rejects_standardization(self):
        value = base_config()
        value["data"]["use_L1pixel_normalized_values"] = True
        value["training"]["standardize"] = True

        with self.assertRaisesRegex(ConfigurationError, "must use training.standardize: false"):
            self.load(value)

    def test_original_reflectance_requires_standardization(self):
        value = base_config()
        value["data"]["use_L1pixel_normalized_values"] = False
        value["training"]["standardize"] = False

        with self.assertRaisesRegex(ConfigurationError, "must use training.standardize: true"):
            self.load(value)
    ...
    def test_relative_experiment_folders_are_rejected(self):
        value = base_config()
        value["experiment_folders"]["a"] = "folder-a"
